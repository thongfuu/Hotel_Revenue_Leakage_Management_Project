...
- การแสดงผล: 
    - สีส้มแดง (OTA): แสดงสัดส่วนใหญ่ที่สุด 65.0% (1,117 รายการ) ครอบคลุมพื้นที่กว่า 2 ใน 3 ของวงกลม
    - สีเขียว (Direct): แสดงสัดส่วน 22.5% (387 รายการ) คิดเป็นประมาณ 1 ใน 5 ของการยกเลิกทั้งหมด
    - สีเหลือง (Wholesale): แสดงสัดส่วนน้อยที่สุด 12.5% (214 รายการ)
- เหตุผลที่เลือกใช้ Donut Chart:
เหมาะสมกับการแสดงสัดส่วนทำให้ผู้อ่านเห็นทันทีว่า OTA ครองสัดส่วนการยกเลิกเมื่อเทียบกับช่องทางอื่น

![H2_graph1](docs/images/H2_graph2.0.png)
...
**Data Visualization:**\
Graph1: Stacked Bar Chart
- ชื่อกราฟ: H2.2 — สัดส่วนสถานะการจอง เปรียบเทียบกลุ่ม OTA vs Direct
- การแสดงผล:
    - แกน X: แสดงประเภทของช่องทางการจอง (Channel Type) แบ่งเป็น 3 กลุ่ม ได้แก่ Direct, OTA และ Wholesale
    - แกน Y: แสดงสัดส่วนเป็นเปอร์เซ็นต์ (%) ตั้งแต่ 0 ถึง 100%
    - แท่งสีเขียว (Checked-In): แสดงสัดส่วนลูกค้าที่เข้าพักจริง — Direct 82.2%, OTA 73.9%, Wholesale 90.3%
    - แท่งสีเหลือง (Cancelled): แสดงอัตราการยกเลิก — Direct 14.3%, OTA 24.5%, Wholesale 7.9%
    - แท่งสีแดง (No-Show): แสดงอัตราการไม่เข้าพักโดยไม่แจ้ง — Direct 3.5%, OTA 1.6%, Wholesale 1.8%
- เหตุผลที่เลือกใช้:
เหมาะสมที่สุดสำหรับการเปรียบเทียบโครงสร้างสัดส่วน

![H2_graph2](docs/images/H2_graph2.1.png)

Graph2: Horizontal Stacked Bar Chart
- ชื่อกราฟ: H2.2 — พฤติกรรมช่วงเวลาที่ยกเลิกแยกตามช่องทาง
- การแสดงผล:
    - แกน Y: แสดงประเภทช่องทางการจอง (Channel Type) ได้แก่ Wholesale, OTA และ Direct
    - แกน X: แสดงสัดส่วนเปอร์เซ็นต์ (%) ตั้งแต่ 0 ถึง 100 โดยทุกแท่งรวมกันเป็น 100% เสมอ
    - แท่งสีแดง (กระชั้นชิด 0-6 วัน): แสดงสัดส่วนการยกเลิกวิกฤต — Wholesale สูงที่สุดถึง 68.2%, Direct 46.5%, OTA 30.8%
    - แท่งสีเหลือง (ระยะกลาง 7-30 วัน): แสดงสัดส่วนระดับกลาง — OTA 30.5%, Direct 26.9%, Wholesale 15.0%
    - แท่งสีเขียว (ล่วงหน้านาน > 30 วัน): แสดงสัดส่วนการยกเลิกที่โรงแรมรับมือได้ — OTA สูงที่สุดที่ 38.7%, Direct 26.6%, Wholesale 16.8%
- เหตุผลที่เลือกใช้ :
เหมาะสมกับการเปรียบเทียบโครงสร้างพฤติกรรมการยกเลิก

![H2_graph3](docs/images/H2_graph2.2.png)
...
![H3_graph1](docs/images/H3_graph.png)
...
- ชื่อกราฟ: H4.1 — การเปรียบเทียบยอดจองเฉลี่ยกับอัตราการยกเลิกรายเดือน
- การแสดงผล:
    - แกน X: แสดงเดือน (month_name) ตั้งแต่ Jan ถึง Dec ครบทั้ง 12 เดือน
    - แกน Y ฝั่งซ้าย (สีน้ำเงิน): แสดงยอดจองเฉลี่ยต่อเดือน (Check-ins) หน่วยเป็นจำนวนครั้ง ตั้งแต่ 0 ถึงประมาณ 500
    - แกน Y ฝั่งขวา (สีแดง): แสดงอัตราการยกเลิก (%) ตั้งแต่ 0 ถึงประมาณ 20%
    - แท่งกราฟสีเขียว: แสดงยอดจองเฉลี่ยในแต่ละเดือน โดย Oct มียอดสูงสุด (~495) และ Dec มียอดต่ำสุด (~215)
    - เส้นกราฟสีแดงพร้อมจุดกลม: แสดงอัตราการยกเลิกรายเดือน โดย Apr สูงสุดที่ 19.3% และ Jul ต่ำสุดที่ 14.9% ป้ายกำกับเปอร์เซ็นต์แสดงครบทุกจุดบนเส้น
- เหตุผลที่เลือกใช้:
เหมาะสมที่สุดสำหรับการนำเสนอข้อมูล 2 มิติที่มีหน่วยวัดต่างกันโดยสิ้นเชิง (จำนวนครั้ง vs เปอร์เซ็นต์)

![H4_graph1](docs/images/H4_graph4.1.png)

Graph2: Bar Chart
- ชื่อกราฟ: H4.2: อัตราการยกเลิกแยกตามฤดูกาล (Cancellation by Season)
- การแสดงผล:
    - แกน X: แสดง "ฤดูกาล (Season)" แบ่งเป็น 3 กลุ่ม ได้แก่ High Season, Low Season และ Shoulder Season
    - แกน Y: แสดง "อัตราการยกเลิก (%)" สเกลตั้งแต่ 0.0 ถึง 20.0%
    - แท่งกราฟสีชมพูกุหลาบ (ทั้ง 3 แท่ง): แสดงอัตราการยกเลิกในแต่ละฤดูกาล โดย High Season อยู่ที่ 16.8%, Low Season อยู่ที่ 17.3% และ Shoulder Season อยู่ที่ 17.4%
- เหตุผลที่เลือกใช้ Bar Chart:
เหมาะสมกับการเปรียบเทียบค่าตัวเลขเดี่ยว ระหว่างกลุ่มที่มีจำนวนน้อย

![H4_graph2](docs/images/H4_graph4.2.png)
...
**Data Visualization:**\
1.ชื่อกราฟ: H5: Relationship between Booking Volume and Cancel Rate by Country
การแสดงผล:

แกน X: แสดง "Total Bookings (Sample Size)" หรือจำนวนการจองทั้งหมดของแต่ละประเทศ/กลุ่ม สเกลตั้งแต่ 0 ถึงกว่า 4,000 การจอง
แกน Y: แสดง "Cancellation Rate (%)" หรืออัตราการยกเลิก สเกลตั้งแต่ประมาณ 15% ถึง 19%
ขนาดของวงกลม (Bubble Size): แทนค่า "avg_lead_time" หรือระยะเวลาจองล่วงหน้าเฉลี่ยของแต่ละกลุ่ม ยิ่งวงกลมใหญ่ยิ่งมี Lead Time นานกว่า (สเกล 44.0–47.2 วัน)
สีน้ำเงินเข้ม (Dark Blue): ตัวแทนกลุ่ม "2. Short-haul (Asia)" มีจุดเดียวที่ Total Bookings ~1,400 และ Cancel Rate สูงที่สุดในกราฟที่ ~19%
สีเขียวเข้ม (Teal/Dark Green): ตัวแทนกลุ่ม "1. Domestic" มี 2 จุด โดยจุดที่มี Bookings สูงสุด (~4,000) มี Cancel Rate อยู่ที่ ~17.3% ซึ่งอยู่เหนือค่าเฉลี่ยเล็กน้อย
สีเขียวอ่อน (Light Green): ตัวแทนกลุ่ม "3. Long-haul (Intercontinental)" มีหลายจุดกระจายตัว โดยส่วนใหญ่มี Cancel Rate ต่ำกว่าค่าเฉลี่ย
เส้นประสีแดง (Avg Cancel Rate): เส้นอ้างอิงที่ ~17.2% แสดงค่าเฉลี่ย Cancel Rate รวม ช่วยให้เห็นว่ากลุ่มใดอยู่เหนือหรือต่ำกว่ามาตรฐาน

เหตุผลที่เลือกใช้:

Bubble Chart สามารถแสดงข้อมูล 4 มิติพร้อมกันในกราฟเดียว ได้แก่ ปริมาณการจอง (X), อัตราการยกเลิก (Y), ระยะเวลาจองล่วงหน้า (ขนาด Bubble) และประเภท Market Segment (สี) ทำให้ทีมบริหารสามารถระบุได้ทันทีว่ากลุ่มลูกค้าใดมีความเสี่ยงสูงทั้งในแง่อัตราการยกเลิกและพฤติกรรมการจอง

2.ชื่อกราฟ: H5.3: Risk Heatmap - ช่องทางจอง vs นโยบาย (Domestic & Short-haul)

การแสดงผล:

แกน X: แสดงประเภทนโยบายการยกเลิก
แกน Y: แสดงช่องทางการจอง
สีใน Heatmap: แสดงอัตราการยกเลิก (%) โดยสีแดงเข้มหมายถึงความเสี่ยงสูง
ตัวเลขในแต่ละช่อง: คือค่าอัตราการยกเลิกจริงของแต่ละคู่ Channel และ Policy

จุดสำคัญ:

Fully Flexible (24h) มีอัตราการยกเลิกสูงที่สุด โดยเฉพาะ Expedia และ Booking.com
Strict Non-Refundable มีอัตราการยกเลิกต่ำที่สุดทุกช่องทาง

เหตุผลที่เลือกใช้:

Heatmap ช่วยเปรียบเทียบความเสี่ยงหลายปัจจัยพร้อมกันได้ง่าย
เห็น Pattern ของการยกเลิกชัดเจนจากระดับสี
เหมาะสำหรับวิเคราะห์ว่าช่องทางใดควรใช้นโยบายการจองแบบเข้มงวดมากขึ้น*
...
**Data Visualization:**\
1. ชื่อกราฟ 2: H6: ความเสียหาย "เฉลี่ยต่อหัว" (Avg. Loss per Guest)
การแสดงผล:

แกน X: แสดง "กลุ่มลูกค้า" แบ่งเป็น 3 กลุ่ม คือ ยกเลิกครั้งเดียว, ยกเลิก 2 ครั้ง และ ยกเลิก 3+ ครั้ง (Serial Cancellers)
แกน Y: แสดง "มูลค่าความเสียหาย (บาท)" สเกลตั้งแต่ 0 ถึง 100,000 บาท
แท่งสีเขียว (12,575 บาท): กลุ่ม One-time มีความเสียหายเฉลี่ยต่อคนต่ำที่สุด
แท่งสีเหลือง (39,917 บาท): กลุ่มยกเลิก 2 ครั้ง มีความเสียหายเฉลี่ยสูงขึ้นกว่า 3 เท่าของกลุ่มแรก
แท่งสีแดง (97,066 บาท): กลุ่ม Serial Cancellers มีความเสียหายเฉลี่ยต่อหัวสูงที่สุด เกือบ 8 เท่าของกลุ่ม One-time สะท้อนถึงพฤติกรรมที่ทำความเสียหายซ้ำซาก

เหตุผลที่เลือกใช้:

Bar Chart แบบเปรียบเทียบ 3 กลุ่มช่วยให้เห็น "ความอันตรายที่แท้จริงต่อหัว" ของลูกค้าแต่ละประเภทได้อย่างชัดเจน เป็นมุมมองที่เสริมกับ Pie Chart เพื่อสื่อสารว่าแม้ Serial Cancellers จะมีจำนวนน้อย แต่ถ้าสามารถป้องกันได้ก็คุ้มค่าอย่างมาก เนื่องจากมูลค่าความเสียหายต่อรายสูงมาก

2.ชื่อกราฟ 1: H6: สัดส่วนความเสียหายรวม แยกตามกลุ่มลูกค้า (Hypothesis Rejected)
การแสดงผล:

ประเภทกราฟ: Pie Chart แสดงสัดส่วนของลูกค้าแต่ละกลุ่มต่อความเสียหายรวมทั้งหมด
ชิ้นสีเขียว (54.66%): ตัวแทนของกลุ่ม "ยกเลิกครั้งเดียว (One-time)" ซึ่งเป็นกลุ่มที่ใหญ่ที่สุด มีจำนวนลูกค้ามากที่สุด (1,176 ราย) และรับผิดชอบความเสียหายรวมสูงสุดในแง่มูลค่าสะสม
ชิ้นสีเหลือง (30.98%): ตัวแทนของกลุ่ม "ยกเลิก 2 ครั้ง" ซึ่งเป็นกลุ่มขนาดกลาง (210 ราย) แต่มีสัดส่วนความเสียหายสูงกว่าสัดส่วนจำนวนลูกค้าอย่างเห็นได้ชัด
ชิ้นสีแดง (14.35%): ตัวแทนของกลุ่ม "ยกเลิก 3+ ครั้ง (Serial Cancellers)" ซึ่งมีจำนวนลูกค้าน้อยที่สุด (40 ราย) แต่ยังคงสร้างความเสียหายเป็นสัดส่วนที่มีนัยสำคัญ

เหตุผลที่เลือกใช้:

Pie Chart เหมาะสำหรับแสดงให้เห็นว่าความเสียหาย "กระจายตัว" อยู่ในกลุ่มลูกค้ากลุ่มใดมากที่สุด และสื่อสาร Insight สำคัญว่า แม้ Serial Cancellers จะดูน่ากลัว แต่ความเสียหายส่วนใหญ่มาจากลูกค้ายกเลิกครั้งเดียวซึ่งมีจำนวนมาก จึงทำให้ Hypothesis ถูก Rejected
...
**Data Visualization:**\
1. ชื่อกราฟ: H7.1: อัตราการยกเลิก (Cancel Rate) ตามระยะเวลาจองล่วงหน้า
การแสดงผล:

แกน X: แสดง "ระยะเวลาจองล่วงหน้า (Lead Time)" แบ่งออกเป็น 6 กลุ่ม ได้แก่ 0-7 วัน (Last Min), 8-30 วัน, 31-90 วัน, 3-6 เดือน, 6-12 เดือน และ 1 ปีขึ้นไป
แกน Y: แสดง "Cancellation Rate (%)" สเกลตัวเลขตั้งแต่ 0 ถึง 35%
แท่งสีเหลือง: เป็นตัวแทนของอัตราการยกเลิกในแต่ละช่วง Lead Time แสดงให้เห็นว่าอัตราการยกเลิกค่อยๆ เพิ่มขึ้นจาก 4.9% (จองกระชั้นชิด) ไปถึงจุดสูงสุดที่ 29.1% (จอง 3-6 เดือนล่วงหน้า) ก่อนจะลดลงและร่วงสู่ 0.0% ในกลุ่มจอง 1 ปีขึ้นไป
เส้นสีแดง: เป็นตัวแทนของแนวโน้ม (Trend Line) ที่ overlay บนแท่ง ช่วยให้มองเห็น Pattern โค้งของอัตราการยกเลิกได้ชัดเจนยิ่งขึ้น โดยเน้นให้เห็นว่าพฤติกรรมการยกเลิกไม่ได้เป็นเส้นตรง แต่มีจุดพีคอยู่ที่ช่วงกลาง

เหตุผลที่เลือกใช้: Combo Chart (แท่ง + เส้น) ช่วยให้เห็นทั้งค่าตัวเลขรายกลุ่มและทิศทางแนวโน้มในคราวเดียว ทำให้ทีมบริหารสามารถออกแบบนโยบาย เช่น เพิ่มเงื่อนไขการยกเลิกสำหรับการจองในช่วง 3-6 เดือนล่วงหน้าที่มีความเสี่ยงสูงที่สุด

2. ชื่อกราฟ: H7.2: ความสัมพันธ์ระหว่าง “วันจอง” กับ “วันยกเลิก”

การแสดงผล:

แกน X: แสดงจำนวนวันจองล่วงหน้า (Lead Time)
แกน Y: แสดงจำนวนวันก่อนเข้าพักที่มีการยกเลิก (Recovery Window)
สีของ Hexbin: แสดงจำนวน Booking โดยสีเข้มหมายถึงมีจำนวนการจองมาก

จุดสำคัญ:

การจองส่วนใหญ่กระจุกตัวในช่วงจองล่วงหน้าน้อยและยกเลิกใกล้วันเข้าพัก
ยิ่งจองล่วงหน้านาน มีแนวโน้มยกเลิกก่อนวันเข้าพักนานขึ้น

เหตุผลที่เลือกใช้: Hexbin Plot ช่วยลดปัญหาจุดข้อมูลทับกันเมื่อข้อมูลมีจำนวนมาก
เห็นความหนาแน่นและแนวโน้มความสัมพันธ์ของข้อมูลได้ชัดเจน
เหมาะสำหรับวิเคราะห์พฤติกรรมการยกเลิกตามช่วงเวลาการจองล่วงหน้า*
...
**Data Visualization :**\
ชื่อกราฟ: H8.1: มูลค่าความเสียหายจาก No-Show (พุ่งสูงสุดช่วง Weekend)
การแสดงผล:

แกน X: แสดง "วันในสัปดาห์" ซึ่งครอบคลุมทั้ง 7 วัน ตั้งแต่ Monday ถึง Sunday โดยแบ่งออกเป็น 2 กลุ่มที่ชัดเจนตามสีของแท่ง คือ วันธรรมดา (Weekday: จันทร์–พฤหัสบดี และอาทิตย์) และ วันสุดสัปดาห์ (Weekend Peak: ศุกร์–เสาร์)
แกน Y: แสดง "มูลค่าความเสียหายสะสม (บาท)" สเกลตัวเลขตั้งแต่ 0 ถึงกว่า 700,000 บาท
แท่งสีเหลือง: เป็นตัวแทนของวันธรรมดาที่มีมูลค่าความเสียหายจาก No-Show อยู่ในระดับปานกลาง ได้แก่ จันทร์ (275,610), อังคาร (396,115), พุธ (403,670), พฤหัสบดี (403,210) และอาทิตย์ (317,080)
แท่งสีแดง: เป็นตัวแทนของวันสุดสัปดาห์ที่มีมูลค่าความเสียหายสูงที่สุด ได้แก่ ศุกร์ (720,520) และ เสาร์ (661,975) ซึ่งสูงกว่าวันธรรมดาอย่างเห็นได้ชัด

เหตุผลที่เลือกใช้:

แสดงให้เห็นรูปแบบ (Pattern) ความเสียหายรายวันได้ชัดเจน ช่วยให้ทีมบริหารสามารถระบุได้ทันทีว่าวันไหนคือ "วันเสี่ยงสูง" ที่ต้องเพิ่มมาตรการป้องกัน เช่น การเรียกเก็บค่ามัดจำหรือการส่ง reminder เพิ่มเติมในช่วงวันศุกร์–เสาร์
